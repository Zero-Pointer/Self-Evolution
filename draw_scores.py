...
def get_all_score(filepath):
    ...
    if "judge" not in datas.keys():
        datas["judge"] = 0
    scores = [
        datas["predict_bleu-4"],
        datas["predict_rouge-1"],
        datas["predict_rouge-2"],
        datas["predict_rouge-l"],
    ]
    return scores
...
def add_baseline_data(df):
    ...
    # 在第一列插入数据
    # Qwen72B
    new_col_first = pd.DataFrame(
        {"Base": [7.26521, 27.36001900000001, 8.097568, 15.389878]}, index=df.index
    )
    df = pd.concat([new_col_first, df], axis=1)

    # 在最后一列插入数据
    # Qwen72B
    new_col_last = pd.DataFrame(
        {"Qwen72B-Data": [8.899804, 29.015283000000004, 9.651207000000001, 17.519384]},
        index=df.index,
    )
