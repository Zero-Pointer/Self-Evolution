...
class LogCallback(TrainerCallback):
    ...
    def on_log(self, args: "TrainingArguments", state: "TrainerState", control: "TrainerControl", **kwargs) -> None:
        ...
        """
        if state.log_history is not None and len(state.log_history) != 0:
            if 'grad_norm' in state.log_history[-1] and isinstance(state.log_history[-1]['grad_norm'], torch.Tensor):
                state.log_history[-1]['grad_norm'] = state.log_history[-1]['grad_norm'].item()
